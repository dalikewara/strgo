package strgo

const ErrEmpty = "the given string is empty"

const ErrMinLength = "the given string length cannot be less than %v"

const ErrMaxLength = "the given string length cannot be more than %v"

const ErrOnlyContainChars = "the given string can not contain char: %s"

const ErrOnlyContainPrefixChars = "the given string can not contain prefix char: %s"

const ErrOnlyContainSuffixChars = "the given string can not contain suffix char: %s"

const ErrMustContainChars = "the given string must contain char: %s"

const ErrMustContainWords = "the given string must contain word: %s"

const ErrMustNotContainChars = "the given string must not contain char: %s"

const ErrMustNotContainWords = "the given string must not contain word: %s"

const ErrMustNotContainPrefixChars = "the given string must not contain prefix: %s"

const ErrMustNotContainSuffixChars = "the given string must not contain suffix: %s"

const ErrMustBeFollowedByChars = "the char: %s, must be followed with at least one of these characters: %v"

const ErrMayContainCharsOnce = "the char: %s, must be appeared once in the string"

const ErrMayContainWordsOnce = "the word: %s, must be appeared once in the string"
